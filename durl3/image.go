...
func DownloadImg(url string) {
	...
	fmt.Println("\nDownload completed.")
	fmt.Printf("Total download time =[%.2fs]\n", cost)
}
...
func DownloadImgGo() {
	defer func() {
		if err := recover(); err != nil {
			//logs.Error("a", "DownloadImg panic, err: %s",err)
			waitGroup.Done()
		}
	}()
	for img := range imgChan {
		...
		if err != nil {
			//logs.Error("a", "getUrlBody() failded, err: %s",err)
			continue
		}
		...
		if err != nil {
			img.isDownload = false
			imgFailed <- img
			//logs.Error("a", "ioutil.WriteFile() failded, err: %s",err)
			continue
		}
		img.isDownload = true
		cost := time.Since(start).Seconds()
		fmt.Printf("Download:%s Time:%.2fs \n", img.fileName, cost)
	}
	waitGroup.Done()
}
...
func getUrlImgGo() {
	defer func() {
		if err := recover(); err != nil {
			//logs.Error("a", "getUrlImgGo panic, err: %s",err)
			imgWG.Done()
		}
	}()
	...
	for url := range urlChan {
		...
		if err != nil {
			//logs.Error("a", "getUrlResp2() failded, err: %s",err)
		...
		if err != nil {
			//logs.Error("a", "ioutil.ReadAll() failded, err: %s",err)
			continue
		}
		Resp.Body.Close()
...
func tryFailedImg() {
	...
	for i := 0; i < TryImgNum; i++ {
		for n, img := range Tryimg {
			...
			cost := time.Since(start).Seconds()
			fmt.Printf("try Download:%s Time:%.2fs \n", img.fileName, cost)
		}
	}
}
